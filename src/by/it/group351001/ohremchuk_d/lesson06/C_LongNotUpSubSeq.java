...
public class C_LongNotUpSubSeq {
    ...
    // Метод для нахождения максимальной невозрастающей подпоследовательности
    int getNotUpSeqSize(InputStream stream) throws FileNotFoundException {
        ...
        // Вычисление максимальной невозрастающей подпоследовательности
        int result = longestDecSubsequenceLength(m);

        return result;
    }

    // Метод для нахождения длины максимальной невозрастающей подпоследовательности
    int longestDecSubsequenceLength(int numbers[]) {
        ...
        // Вычисление длины подпоследовательности и индексов элементов
        for (int i = 0; i < numbers.length; i++) {
            for (int j = 0; j < i; j++) {
        ...
        for (int i = resSeqind.length - 1; i >= 0; i--) {
            resSeqind[i] = k + 1; // Индекс начинается с 1
            k = prev[k];
        }

        // Вывод элементов на экран
        for (int i = 0; i < resSeqind.length; i++) {
            System.out.print(resSeqind[i] + " ");
        }
        System.out.println();
