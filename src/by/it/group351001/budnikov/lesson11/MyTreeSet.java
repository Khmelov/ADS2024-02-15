...
public class MyTreeSet<E extends Comparable<E>> implements Set<E> {
    ...
    public String toString() {
        ...
        RecursiveStringCompletion(root, resultStr);
        return resultStr.append("]").toString();
    }

    void RecursiveStringCompletion(Node node, StringBuilder sb) {
        if (node == null) return;
        RecursiveStringCompletion(node.left, sb);
        if (sb.length() > 1)
            sb.append(", ");
        sb.append(node.data);
        RecursiveStringCompletion(node.right, sb);
    }
