...
public class C_EditDist {
    ...
    String getDistanceEdinting(String one, String two) {
        ...
        // Инициализация переменных и создание матрицы для хранения значений
        int n = one.length();
        int m = two.length();
        int[][] matrix = new int[n + 1][m + 1];

        // Заполнение матрицы для алгоритма Левенштейна
        for (int i = 0; i <= n; i++) {
            for (int j = 0; j <= m; j++) {
                if ((i == 0) && (j == 0)) {
                    matrix[i][j] = 0;
        ...
        // Формирование редакционного предписания
        String result = "";
        int i = n, j = m;
        while (i > 0 && j > 0) {
        ...
        // Дополнительные операции, если одна строка закончилась раньше другой
    /*
    while (i > 0) {
        result = "-" + one.charAt(i - 1) + "," + result;
        i--;
