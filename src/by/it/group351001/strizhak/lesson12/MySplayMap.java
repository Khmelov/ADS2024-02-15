...
public class MySplayMap implements NavigableMap<Integer, String> {
    ...
    Node splay(Node node, Integer key) {
        ...
        if (cmp < 0) { // Если ключ меньше ключа текущего узла
            if (node.left == null) {
                return node; // Если левый ребенок пуст, возвращаем текущий узел
            }
            int cmp2 = key.compareTo(node.left.key);
            if (cmp2 < 0) { // Если ключ меньше ключа левого ребенка (Zig-Zig)
                node.left.left = splay(node.left.left, key); // Рекурсивно выполняем splay для левого ребенка левого ребенка
                node = rotateRight(node); // Выполняем правый поворот
            } else if (cmp2 > 0) { // Если ключ больше ключа левого ребенка (Zig-Zag)
                node.left.right = splay(node.left.right, key); // Рекурсивно выполняем splay для правого ребенка левого ребенка
                if (node.left.right != null) {
                    node.left = rotateLeft(node.left); // Выполняем левый поворот
                }
            }
            if (node.left == null) {
                return node; // Если левый ребенок пуст, возвращаем текущий узел
        ...
        } else if (cmp > 0) { // Если ключ больше ключа текущего узла
            if (node.right == null) {
                return node; // Если правый ребенок пуст, возвращаем текущий узел
            }
            int cmp2 = key.compareTo(node.right.key);
            if (cmp2 < 0) { // Если ключ меньше ключа правого ребенка (Zag-Zig)
                node.right.left = splay(node.right.left, key); // Рекурсивно выполняем splay для левого ребенка правого ребенка
                if (node.right.left != null) {
                    node.right = rotateRight(node.right); // Выполняем правый поворот
                }
