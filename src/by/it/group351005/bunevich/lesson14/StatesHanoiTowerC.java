package by.it.group351005.bunevich.lesson14;
...
public class StatesHanoiTowerC {
    ...
    public static void main(String[] args) {
        ...
        int max_size = (1 << N) - 1;  //максимальное количество шагов используя побитовый сдвиг (1 << N означает 2 в степени N)
        int[] steps_heights = new int[N]; //отслеживания высот на каждом шагу
        for (int i = 0; i < N; i++)
            steps_heights[i] = -1;
        DSU dsu = new DSU(max_size); //управления объединением множеств
        int[] heights = new int[3]; //кущей высоты каждого из трех стержней.
        heights[0] = N;
        for (int i = 0; i < max_size; i++) {
            ...
            dsu.makeSet(i);
//Вычисляет изменения высот для текущего шага. Использует функцию carryingOver
// для определения перемещений дисков.
           ...
           // Если высота не была ранее записана, записывает текущий шаг.
          //  В противном случае объединяет множества.
        }
