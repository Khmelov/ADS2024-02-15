...
public class A_Huffman {
    ...
    //!!!!!!!!!!!!!!!!!!!!!!!!!     НАЧАЛО ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!!
    //Этот метод выполняет кодирование строки с использованием алгоритма Хаффмана.
    // Он принимает файл, из которого будет считываться строка для кодирования.
    // Затем создается Map count, в котором подсчитывается частота каждого символа в строке.
    // Создается приоритетная очередь priorityQueue, в которую добавляются листья с символами и их частотами из count.
    //  Затем происходит построение дерева Хаффмана путем извлечения двух узлов из очереди,
    //  создания родительского узла и добавления его обратно в очередь.
    // После построения дерева вызывается метод fillCodes на корневом узле для генерации кодов символов.
    // Полученные коды символов сохраняются в индексе codes.
