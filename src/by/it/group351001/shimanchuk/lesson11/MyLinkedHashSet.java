...
// Класс MyLinkedHashSet<E> реализует упорядоченное множество на основе хеш-таблицы.
// Использует двойную связность для поддержки порядка вставки элементов и хеш-таблицу для быстрого поиска и удаления.
// Внутренний класс Node<E> хранит данные элемента и ссылки для хеширования и порядка вставки.
// Основные методы включают:
// - Добавление элемента (add): добавляет элемент, проверяя наличие и поддерживая порядок вставки.
// - Удаление элемента (remove): удаляет указанный элемент, обновляя связи.
// - Проверка наличия элемента (contains): определяет, существует ли элемент в множестве.
// - Метод toString() возвращает строковое представление элементов в порядке вставки.
// - Методы для работы с коллекциями: добавление нескольких элементов (addAll), удаление (removeAll), сохранение элементов (retainAll) и проверка наличия всех элементов (containsAll).
// - Метод clear() очищает множество, удаляя все элементы.
// - Метод resize() увеличивает размер массива и перераспределяет элементы для уменьшения коллизий.
// Не реализованы некоторые методы интерфейса Set, такие как iterator() и toArray().
