package by.it.group351004.kostyukov.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for(Map.Entry<Character, Integer> entry: count.entrySet()) {
            A_Huffman.Node symbol = new A_Huffman.LeafNode(entry.getValue(), entry.getKey());
        ...
        while (priorityQueue.size() > 1) {
            A_Huffman.Node leftSymbol = priorityQueue.remove();
            A_Huffman.Node rightSymbol = priorityQueue.remove();
            A_Huffman.Node parentsSymbol = new A_Huffman.InternalNode(leftSymbol, rightSymbol);
        ...
        A_Huffman.Node root = priorityQueue.remove();
        root.fillCodes("");
        for (int i = 0; i < s.length(); i++) {
