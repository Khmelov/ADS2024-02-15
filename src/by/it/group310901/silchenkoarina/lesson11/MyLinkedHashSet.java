...
public class MyLinkedHashSet<E> implements Set<E> {
    ...
    public boolean addAll(Collection c) {
        ...
        for(int i = 0;i < cArray.length;i++){
            result |= add(cArray[i]); // Используем побитовую операцию ИЛИ для объединения результатов.
            //Если хотя бы один вызов метода add() вернет true (т.е. хотя бы один элемент был успешно добавлен), result станет true.
        }
        return result;
    }
    ...
    public boolean retainAll(Collection c) {
        ...
        boolean result = false;
        while(x!=null){
            if(!c.contains(x.data)){ // Если элемент не содержится в переданной коллекции,
                E tempE = x.data; // сохраняем его значение.
                x = x.after;
                remove(tempE);
                result = true;
            }else {
