...
public class A_Huffman {
    ...
    void create(Node node, String code){
        if (node instanceof LeafNode){
            codes.put(((LeafNode) node).symbol, code);
        }
        else {
            create(((InternalNode) node).left, code + '1');
            create(((InternalNode) node).right, code + '0');
        }
    }
    String encode(File file) throws FileNotFoundException {
        ...
        for(var key : count.keySet()) {
            var Leaf = new LeafNode(count.get(key), key);
        ...
        while (priorityQueue.size() != 1){
            var Leaf1 = priorityQueue.peek();
            priorityQueue.remove(Leaf1);
            var Leaf2 = priorityQueue.peek();
            priorityQueue.remove(Leaf2);
            InternalNode NewNode;
            if (Leaf1.frequence < Leaf2.frequence)
                NewNode = new InternalNode(Leaf1, Leaf2);
            else NewNode = new InternalNode(Leaf2, Leaf1);
            priorityQueue.add(NewNode);
        }
        //4. последний из родителей будет корнем этого дерева
        //это будет последний и единственный элемент оставшийся в очереди priorityQueue.
        StringBuilder sb = new StringBuilder();
        create(priorityQueue.peek(), "");
        for (int i = 0; i < s.length(); i ++){
