package by.it.group351003.lisunova.lesson05;
...
скольким отрезкам она принадлежит.
    ...
    т.е. работа камер - промежутки времени
    события - точки
*/
...
public class A_QSort {
    ...
    Segment[] QSort(Segment[] events, int left, int right){
        ...
        int j = right;
        while (i < j) {
            while (events[i].start < mid.start)  // mid - середина
                ++i;
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        //читаем сами отрезки
        // сегмент (начало, конец) т.е. состоит из 2 точек
        for (int i = 0; i < n; i++) {
        ...
        QSort(segments,0,n - 1);  //до числа отрезков отсортированного массива
        int k = 0;
        for (int i = 0; i < m; i++) {
            points[i] = scanner.nextInt();
            int j = 0;
            // смотрим чтобы точка входила в промежуток
