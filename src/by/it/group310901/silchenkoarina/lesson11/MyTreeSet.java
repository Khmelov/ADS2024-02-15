...
public class MyTreeSet<E> implements Set<E> {
    ...
    static private class MyNode<E>{
        E data;
        int hash;
        MyNode<E> left, right; //ссылки на левое и правое поддерево
    ...
    public String toString() {
        ...
        if (root!=null) {
            addtostring(root, sb);
            sb.delete(sb.length()-2,sb.length()); //удаляем лишние запятые и пробелы
        }
        sb.append("]");
        return sb.toString();
    }
    ...
    public boolean add(E e) {
        ...
        while(x!=null){
            if (x.hash==hash)
                return false;
            parent = x; //обновляем родительский узел текущим узлом х
            if (x.hash>hash)
                x = x.left;
            else
                x = x.right;
    ...
    public boolean retain(MyNode node, Collection<?> c){
        ...
        if(!c.contains(node.data)) {
            result = remove(node.data); // Удаление узла, если его нет в коллекции `c`
        }
        return result;
    }
    @Override
